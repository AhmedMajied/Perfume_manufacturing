...
public class ExcelUtility {
	...
	public void generate_needed_materials_report() {
		...
		try{
			...
			write_needed_liquids(DBConnection.retrieve_reorder_materials("Liquid"),0);
			write_needed_liquids(DBConnection.retrieve_reorder_materials("Flavor"),4);
			write_needed_liquids(DBConnection.retrieve_reorder_materials("Bottle"),8);
	...
	private void write_needed_liquids(Vector<Material> needed_materials,int start_col){
		for(int row=0;row<needed_materials.size();row++) {
			try {
				...
				sheet.addCell(new Number(start_col+1,row+2,needed_materials.get(row).get_total_quantity(),data_cell_format));
